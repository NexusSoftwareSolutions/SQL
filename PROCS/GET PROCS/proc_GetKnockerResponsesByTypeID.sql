...
-- Description:	Get KnockerResponses by KnockResponseTypeID
-- =============================================
ALTER PROCEDURE proc_GetKnockerResponsesByTypeID
...
BEGIN
	...
	IF @KnockResponseTypeID is null
		BEGIN
			SET FMTONLY ON;
	...
	ELSE
		BEGIN
			IF NOT EXISTS (SELECT KnockerResponseID FROM KnockerResponses
							WHERE KnockResponseTypeID = @KnockResponseTypeID)
				BEGIN
					BEGIN TRY
